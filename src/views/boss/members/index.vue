...
const depositRecords: DepositRecord[] = [{ amount: 300, gift: 30, time: '2026-03-20 10:00' }]
...
const levelForm = reactive<Omit<Level, 'id'> & { id?: number }>({
  name: '',
  discount: 1.0,
  upgradeCondition: '',
})
      ...
      <template v-if="currentMember">
        <el-descriptions :column="2" border style="margin-bottom: 16px">
          ...
          <el-descriptions-item label="余额"
            >￥{{ currentMember.balance.toFixed(2) }}</el-descriptions-item
          >
          <el-descriptions-item label="积分">{{ currentMember.points }}</el-descriptions-item>
          <el-descriptions-item label="注册时间" :span="2">{{
            currentMember.registerTime
          ...
          <el-tab-pane label="储值记录" name="deposit">
            <el-table :data="depositRecords" size="small" border>
              ...
              <el-table-column label="赠送金额"
                ><template #default="{ row }">￥{{ row.gift }}</template></el-table-column
              >
              <el-table-column label="时间" prop="time" />
            </el-table>
          </el-tab-pane>
